...
float gyro_dps[3];    // 角速度，单位：度/秒
float mag_ut[3];      // 磁场强度，单位：微特斯拉
uint8_t test;
...
void ICM20948_READ_MAG()
{
	...
	mag_data[0] = mag_buffer[1] | mag_buffer[2] << 8;
	mag_buffer[3] = ICM_Mag_Read(0x13);
	mag_buffer[4] = ICM_Mag_Read(0x14);
	mag_data[1] = mag_buffer[3] | mag_buffer[4] << 8;
	mag_buffer[5] = ICM_Mag_Read(0x15);
	mag_buffer[6] = ICM_Mag_Read(0x16);
	mag_data[2] = mag_buffer[5] | mag_buffer[6] << 8;

	// 磁力计转换为微特斯拉(μT)
    mag_ut[0] = (float)mag_data[0] * 4900.0f / 32768.0f;
    mag_ut[1] = (float)mag_data[1] * 4900.0f / 32768.0f;
    mag_ut[2] = (float)mag_data[2] * 4900.0f / 32768.0f;
...
void ICM_ReadAccelGyro(void)
{
	...
	gyro_data[2] = (raw_data[10] << 8) | raw_data[11];
	    ...
	    // 加速度转换为g (重力加速度单位)
    accel_g[0] = (float)((int16_t)accel_data[0]) * 8.0f / 32768.0f;
    accel_g[1] = (float)((int16_t)accel_data[1]) * 8.0f / 32768.0f;
    accel_g[2] = (float)((int16_t)accel_data[2]) * 8.0f / 32768.0f;
    
    // 陀螺仪转换为度/秒
    gyro_dps[0] = (float)((int16_t)gyro_data[0]) * 250.0f / 32768.0f;
    gyro_dps[1] = (float)((int16_t)gyro_data[1]) * 250.0f / 32768.0f;
    gyro_dps[2] = (float)((int16_t)gyro_data[2]) * 250.0f / 32768.0f;
}
